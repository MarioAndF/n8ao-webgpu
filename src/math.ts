...
export type N8AODepthType =
  (typeof DepthType)[keyof typeof DepthType];
...
export function createDefaultN8AOConfiguration(): N8AOConfiguration {
  return {
    aoSamples: 16,
    aoRadius: 5,
    aoTones: 0,
...
export function generateHemisphereSamples(sampleCount: number): Vector3[] {
...
export function generateDenoiseSamples(
  numSamples: number,
  numRings: number,
): Vector2[] {
  ...
  for (let index = 0; index < numSamples; index += 1) {
    samples.push(
      new Vector2(Math.cos(angle), Math.sin(angle)).multiplyScalar(
...
export function applyQualityMode(
  configuration: N8AOConfiguration,
  mode: N8AOQualityMode,
): void {
  if (mode === "Performance") {
...
export function resolveDisplayMode(
  mode: N8AODisplayMode,
): 0 | 1 | 2 | 3 | 4 {
  return ["Combined", "AO", "No AO", "Split", "Split AO"].indexOf(
    mode,
  ) as 0 | 1 | 2 | 3 | 4;
}
